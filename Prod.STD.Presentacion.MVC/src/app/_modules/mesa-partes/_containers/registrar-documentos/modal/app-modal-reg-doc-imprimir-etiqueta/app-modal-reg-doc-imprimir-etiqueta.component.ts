...
export class AppModalRegDocImprimirEtiquetaComponent implements OnInit {
    id_documento: number;
    ...
    imprimirEtiquetaAdministrado = () => {
        window.open(
            `/api/mesa-partes/registrar-documento/documento/${
                this.id_documento
            }/etiqueta-administrado`,
            "window",
    ...
    imprimirEtiquetaProduce = () => {
        window.open(
            `/api/mesa-partes/registrar-documento/documento/${this.id_documento}/etiqueta-produce`,
            "window",
            "menubar=0,toolbar=0,scrollbars=yes,location=0,directories=0,status=0,resizable=0,top=0,left=0,width=560,height=400"
        );
    };
